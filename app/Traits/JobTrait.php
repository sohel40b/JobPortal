...
trait JobTrait
{
    ...
    public function viewJobType(){
        ...
        if ((bool)$company->industry_id === false) {
            flash(__('Please Create Your Account Profile!!'))->error();
            flash(__('Example:- Goto==> Create/Edit Profile'))->error();
            return \Redirect::route('company.home');
            exit;
        }
        
        return view('job.job_post_type');
    }
    ...
    private function assignMatchingValues($matchingDataMailProcess, $request)
    {
        $matchingDataMailProcess->job_functional_area_id = $request->input('functional_area_id');
        return $matchingDataMailProcess;
    }
    ...
    public function storeFrontJob(JobFrontFormRequest $request)
    {
        ...
        flash(' Catagories Job has been added! Please Wait.... Admin check your post and Live your Job Post as soon as possible')->success();
    ...
    public function updateFrontJob($id, JobFrontFormRequest $request)
    {
        ...
        /*         * ************************************ */
        /*         * ************************************ */
        $matchingDataMailProcess = MatchingDataMailProcess::find($id);
        $matchingDataMailProcess = MatchingDataMailProcess::where('job_id', $id)->first();
        $matchingDataMailProcess = $this->assignMatchingValues($matchingDataMailProcess, $request);
        $matchingDataMailProcess->save();
        /*         * ************************************ */
        flash('Catagories Job has been updated!')->success();
        return \Redirect::route('edit.front.job', array($job->id));
    }

    public function createHotJob()
    {
        $company = Auth::guard('company')->user();
    ...
    public function storeHotJob(JobFrontFormRequest $request)
    {
        $company = Auth::guard('company')->user();
        ...
        event(new JobPosted($job));
        flash('Hot Job has been added! Please Wait.... Admin check your post and Live your Job Post as soon as possible')->success();
        return \Redirect::route('edit.hot.job', array($job->id));
    }

    public function editHotJob($id)
    {
        $countries = DataArrayHelper::langCountriesArray();
        $currencies = DataArrayHelper::currenciesArray();
        $careerLevels = DataArrayHelper::langCareerLevelsArray();
    ...
    public function updateHotJob($id, JobFrontFormRequest $request)
    {
        $job = Job::findOrFail($id);
		$job = $this->assignJobValues($job, $request);
        /*         * ******************************* */
        ...
        $matchingDataMailProcess->save();
        /*         * ************************************ */
        flash('Hot Job has been updated!')->success();
        return \Redirect::route('edit.hot.job', array($job->id));
    }
