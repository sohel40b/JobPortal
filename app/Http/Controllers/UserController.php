...
class UserController extends Controller
{
    ...
    {
        //$this->middleware('auth', ['only' => ['myProfile', 'updateMyProfile', 'viewCvProfile']]);
        $this->middleware('auth', ['except' => ['showApplicantProfileEducation', 'showApplicantProfileExperience', 'showApplicantProfileSkills', 'showApplicantProfileLanguages']]);
    }
    ...
    public function updateMyProfile(UserFrontFormRequest $request)
    {
        ...
        if($user->update()){
            $matchingUser = MatchingDataMailProcess::where('user_id', 'like', Auth::user()->id)->first();
            if ($matchingUser === null) {
                // user doesn't exist
                $matchingUser = Auth::user();
                $user_id = $matchingUser->id;
                $matchingUser = new MatchingDataMailProcess();
