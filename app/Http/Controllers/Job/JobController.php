...
class JobController extends Controller
{
    ...
    {
        $this->middleware('auth', ['except' => ['jobsBySearch', 'hotJobsBySearch', 'jobDetail', 'hotJobDetail']]);
    ...
    public function hotJobsBySearch(Request $request)
    {
        $search = $request->query('search', '');
        $job_titles = $request->query('job_title', array());
        $company_ids = $request->query('company_id', array());
    ...
    public function hotJobDetail(Request $request, $job_slug)
    {
        ...
        return view('job.hot_job_detail')
                        ->with('job', $job)
                        ->with('relatedJobs', $relatedJobs)
                        ->with('seo', $seo);
    }
    ...
    public function applyJob(Request $request, $job_slug)
    {
        ...
        if ((bool)$user->date_of_birth === false) {
            flash(__('Please Create Your Personal Information!!'))->error();
            flash(__('Example:- Goto==> Manage Resume'))->error();
            return \Redirect::route('job.detail', $job_slug);
            exit;
        }
